...
코틀린 또한 JVM을 위한 언어로 설계되었기 때문에 역시 소거의 개념이 존재합니다. 게다가 코틀린은 타입에 대해 보다 더 엄격한 언어이기 때문에 소거에 민감한데, 간단한 예를 하나 살펴보겠습니다. 아래의 코드는 아무 문제 없이 동작할 것 같지만 컴파일 에러를 발생시킵니다. 런타임에 타입 T에 대한 어떠한 타입 정보도 제공 할 수 없기 때문입니다.
